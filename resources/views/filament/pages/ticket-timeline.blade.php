            ...
            <x-filament::section>
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <h2 class="text-lg font-medium text-gray-900 dark:text-white">
                        {{ $selectedProject->name ?? __('Select Project') }}
                    </h2>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow">
                <div class="p-4 border-b border-gray-200 dark:border-gray-700">
                    <div class="flex items-center gap-2">
                        ...
                        <h2 class="text-lg font-medium text-gray-900 dark:text-white">{{__('Project Timeline')}}</h2>
                    </div>
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                        {{__('Timeline view showing ticket duration from start to due date')}}
                    </p>
                </div>
                                ...
                                <td class="px-4 py-4 border-r border-gray-200 dark:border-gray-600">
                                    <div class="flex flex-col">
                                        ...
                                        <div class="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400 mt-1">
                                            ...
                                            <span>{{__('Due')}}: {{ $task['end_date'] }}</span>
                                        </div>
                                    </div>
                                </td>
                                            ...
                                            <div
                                                ...
                                                <div
                                                    ...
                                                    <div class="fixed hidden group-hover:flex flex-col bottom-auto left-auto transform translate-y-[-100%] mt-[-10px] px-3 py-2 bg-gray-900 dark:bg-gray-700 text-white text-xs rounded-md whitespace-nowrap z-50 shadow-lg">
                                                        ...
                                                        <span>{{__('ID')}}: {{ $task['ticket_id'] }}</span>
                                                        <span>{{__('Due')}}: {{ $task['end_date'] }}</span>
                            ...
                            <tr>
                                <td colspan="{{ count($this->getMonthHeaders()) + 1 }}"
                                    ...
                                    <div class="flex flex-col items-center justify-center mb-3">
                                        ...
                                        <span>{{__('No tickets found with due dates')}}</span>
                                        <span class="text-xs mt-1">{{__('Select a different project or add tickets with due dates')}}</span>
                                    </div>
                                </td>
                            </tr>
                        @endif
        ...
        @else
            <div class="flex flex-col items-center justify-center h-64 text-gray-500 dark:text-gray-400 gap-4">
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400">
                    {{__('Select a project from the dropdown above to view the timeline')}}
                </p>
            </div>
        @endif
    </div>
